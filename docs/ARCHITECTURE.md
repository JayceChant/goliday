...
NewCalendar：为每年构建 yearIndex{off, work, festival} 三个
             ...
             （长度 = 年天数，uint8 按组合计数存储，跨年累加转 int）
      ▼
注册路由 → ListenAndServe
```
...
- **按值计数前缀和**：`NewCalendar` 为每年构建长度 = 年天数的前缀数组（`prefix[i]` 为闭区间 `[元旦, 元旦+i 天]` 的 5 种合法细粒度值累计天数，五值 MECE；元素 uint8 存储——年内单一类型天数有界，跨年累加先转 int 防溢出），构建 O(年天数) 一次完成、构建后只读；统计为年内 O(1) 差分（左闭右开查询转闭区间下标，下标 -1 视为全零）、跨年 O(覆盖年数) 拆段相加，因此 `/stats` 无需限制查询跨度。
