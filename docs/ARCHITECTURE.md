...
Calendar.Query（逐日，明细/单日判定）：
    稀疏终态表命中 → 构建期按「节日 > 补班 > 调休」优先级
    烘焙的终态（节日当天 → Rest|Festival、work → Work|Compensate、
    off → Rest|Adjusted）
...
- **穷举不变量**：`DayType` 为 uint8 小域，`TestDayTypeExhaustiveInvariants` 遍历全部 256 个取值验证 `String` 输出收敛于已知标签（合法值标签 / `unknown` / `invalid`）且不 panic、`IsValid` 恰对 {1,2,6,10,17} 为 true，对 5 个合法值另验证 `Coarse` ∈ {Work, Rest} 且幂等、`IsWork/IsRest` 恰一为真（非法值全 false）、位与判类恰一非零；`TestCalendarConfiguredYearExhaustive` 对已配置年份全年逐日验证判型 ∈ 5 种合法值。
...
| [release-please](https://github.com/JayceChant/goliday/actions/workflows/release-please.yml) | 自动化版本发布：依 Conventional Commits 维护 Release PR 与 CHANGELOG.md，Release PR 合并后自动打 tag、建 GitHub Release 并 dispatch docker.yml 发布 GHCR 镜像（[工作流](../.github/workflows/release-please.yml)） |
