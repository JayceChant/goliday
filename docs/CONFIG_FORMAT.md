...
`DayType` 为 `uint8` 位掩码，采用「终态双层」编码：基本位互斥恰一（当日最终是否上班），调整位互斥至多一（依附基本位）；全部组合的 `|` 均为 all-of（合取）语义，粗粒度即基本位投影（`t & dayTypeCoarseMask`，内部掩码 = `Work|Rest` = 3，不导出）：

| 常量 | 数值 | 二进制 | 层 | 含义 |
|---|---|---|---|---|
| `DayTypeUnknown` | 0 | `0b00000` | 零值 | 未定义类型（默认值）；非合法取值，`String()` 输出 `unknown` |
...
细→粗投影（`Coarse()`）：`t & dayTypeCoarseMask`（内部常量），单次按位与；调整位投影与之对应（`Adjustment()`）：`t & dayTypeAdjustMask`（内部常量 = 28），合法值上 ∈ {0, 4, 8, 16}；判类仅需 `t & 1 != 0`（上班）/ `t & 2 != 0`（放假），无优先级消歧。
...
>
> 注 2：`String()` 对合法值查静态标签表直返（标签与常量名逐字对应），故 `FestivalRest`（6）输出 `rest|festival`，`AdjustedWorkDay`（17）输出 `work|adjusted_work`；粗粒度值 1/2 天然输出 `work`/`rest`；非法值统一输出 `invalid`（配置校验保证不会产生）。
>
