...
5. **festival 仅 name + date**：只记节日当天（如春节只记正月初一那天，不记除夕、不记整个假期区间）；它与 off/work 相互独立——节日当天恰逢周末时仍只写 festival、不写 off/work。
...
> 注 1：该全集的前提是配置符合真实方案——节日当天要么是工作日（必在 `off`，得 24）、要么恰逢自然周末（得 12）；校验规则 8（work 不含节日当天）保证了不会出现「补班 × 节日」的组合。
>
